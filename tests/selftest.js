 ...
 * Every content locale is graded, because a translation can override the code a
 * task expects to see.
 *
 * Query parameters: ?locale=<id>, ?lesson=<id>, ?from=<number>&to=<number> to
 * narrow the run, ?starters=0 to skip the starter pass.
 */
...
const locales = params.get('locale') ? [params.get('locale')] : CONTENT_LOCALES;
...
async function runTask(locale, lesson, task, block) {
  ...
  bad.forEach((result) => {
    failures.push(`[${locale}] ${lesson.number}.${task.position} ${task.id} → ${result.label}: ${result.message}`);
  ...
  if (checkStarters) {
    ...
    if (starterResults.length === task.checks.length && starterResults.every((result) => result.ok)) {
      ok = false;
      failures.push(`[${locale}] ${lesson.number}.${task.position} ${task.id} → стартовый код уже проходит все проверки`);
...
async function run() {
  ...
  for (const locale of locales) {
    // The locale is global state inside the app: it picks the sandbox message
    // dictionary, so the run has to switch it, not just read another course.
    setLocale(locale);
    ...
    heading.textContent = `Локаль: ${locale}`;
    ...
    for (const lesson of lessons) {
      ...
      for (const task of lesson.tasks) {
        await runTask(locale, lesson, task, block);
  ...
  window.__SELFTEST__ = { ok, locales, totalTasks, passedTasks, checked, failures, seconds };
